...
public:
    ...
    void group_load_directedGraph(std::string path, count_type vertices)
	{
		...
		memset(outDegree, 0, vertexNum * sizeof(offset_type));

		if (graphRepresentation == GraphRepresentation::CSR)
		{
#ifdef INDEGREE
			inDegree = new offset_type[vertexNum];
			memset(inDegree, 0, vertexNum * sizeof(offset_type));
#endif
		}
		...
		while (read_bytes < bytes_to_read) {
			...
			if (bytes_to_read - read_bytes > edge_size* chunk_size) { // A chunk_size can store up to 2^20 edges
			...
			if(edgeNum > 100000000)
			{
				...
				{
					rate = rate + 0.1;
					Msg_rate("Graph Bin Load Has Finsihed %.2lf %%", 
						static_cast<double>(curr_read_bytes) / static_cast<double>(bytes_to_read) * 100);
				}
			}
			...
			omp_parallel_for(uint64_t e_i = 0; e_i < curr_read_edges; e_i++)
			{
				...
				if (std::numeric_limits<offset_type>::max() <= (outDegree[src])){
					assert_msg(false, "<offset_type> is not enough to hold [outDegree] array, outDegree[%zu] = %zu", static_cast<uint64_t>(src), static_cast<uint64_t>(outDegree[src]));
				}
#endif
				__sync_fetch_and_add_8(&outDegree[src], static_cast<offset_type>(1));//__sync_fetch_and_add_8(&outDegree[src], 1);//_8

				//> Get inDegree
#ifdef INDEGREE
	#ifdef DATA_TYPE_CHECK
					if (std::numeric_limits<offset_type>::max() <= (inDegree[dst])){
						assert_msg(false, "<offset_type> is not enough to hold [inDegree] array");
					}
	#endif 
					__sync_fetch_and_add_8(&inDegree[dst], static_cast<offset_type>(1));//__sync_fetch_and_add_8(&inDegree[dst], 1);//_8				
#endif
				...
				{
				// In CSC, Weather Define "INDEGREE", [inDegree] Need Compute, If Has Compute, We don't have to do it again.
#ifndef INDEGREE
	#ifdef DATA_TYPE_CHECK
					if (std::numeric_limits<offset_type>::max() <= (inDegree[dst])){
						assert_msg(false, "<offset_type> is not enough to hold [inDegree] array");
	...
	#endif
					__sync_fetch_and_add_8(&inDegree[dst], 1);//__sync_fetch_and_add_8(&inDegree[dst], 1);//_8
#endif
	...
	inline static bool cmp(nbr_pair_type a, nbr_pair_type b) {
	...
	void check_sortNbr()
	{
		omp_parallel_for(count_type i = 0; i < vertexNum; i++)
		{
			...
			if (nbr.size() > 1)
			{
				for (offset_type j = 0; j < (nbr.size() - 1); j++)
				{
					if (nbr[j].first >= nbr[j + 1].first) printf("vertex = %d, nbr[j].first = %d,  nbr[j + 1].first=%d\n", i, nbr[j].first, nbr[j + 1].first);
...
private:
	...
	void build_csr()
	{
		if (graphRepresentation != GraphRepresentation::CSR) assert_msg(false, "Current GraphRepresentation Can Not Build CSR");

		if (!csr)
		{
			if (adjlist.empty()) assert_msg(false, "adjList Vector Empty");
	...
	void build_csc()
	{
		if (graphRepresentation != GraphRepresentation::CSC) assert_msg(false, "Current GraphRepresentation Can Not Build CSC");
...
public:
	 ...
	 {
		if (adjlist.size() == 0) assert_msg(false, "adjlist.size() = 0, adjList may be already cleared");
		if (edgeList.size() != 0) assert_msg(false, "edgeList.size() != 0, The initial edgeList must be the empty ");
	...
	void printAdjList()
	{
		for (count_type i = 0; i < vertexNum; i++)
		{
			neighbor_type nbr = adjlist[i];
			printf("[%d]:", i);
			for (offset_type j = 0; j < nbr.size(); j++)
			{
