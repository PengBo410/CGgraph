...
std::string getResultFilePath(CheckInfo_type& checkInfo)
{
	...
	{
		rightFile = "..." + checkInfo.graphName + "_" + getAlgName(checkInfo.algorithm) + "_" + std::to_string(checkInfo.root) + ".bin";

		bool isExist = (access(rightFile.c_str(), F_OK) >= 0);
		if (!isExist)
		{
			assert_msg(false, "no [%s]", rightFile.c_str());
		}
...
void checkBinResult(std::string resultFilePath, T* current_array, size_t length, OrderMethod orderMethod, T* old2new)
{
	...
	if (orderMethod == OrderMethod::NATIVE)
	{
		omp_parallel_for(count_type i = 0; i < length; i++)
		{
			...
			{
				assert_msg((current_array[i] == resultArray[i]), "error: current_array[%u] = %u, resultArray[%u] = %u",
					i, current_array[i], i, resultArray[i]);
			}
			else if constexpr (std::is_same_v<T, float>)
			{
				assert_msg((current_array[i] == resultArray[i]), "error: current_array[%u] = %f, resultArray[%u] = %f",
					i, current_array[i], i, resultArray[i]);
			}
			else if constexpr (std::is_same_v<T, double>)
			{
				assert_msg((current_array[i] == resultArray[i]), "error: current_array[%u] = %lf, resultArray[%u] = %lf",
					i, current_array[i], i, resultArray[i]);
			}
			else
			{
	...
	else
	{
		omp_parallel_for(count_type i = 0; i < length; i++)
		{
			if constexpr (std::is_same_v<T, uint32_t>)
			{
				...
				assert_msg((current_array[old2new[i]] == resultArray[i]), "error:current_array[%u] = %u, resultArray[%u] = %u",
					old2new[i], current_array[old2new[i]], i, resultArray[i]);
			}
			else if constexpr (std::is_same_v<T, float>)
			{
				assert_msg((current_array[old2new[i]] == resultArray[i]), "error:current_array[%u] = %f, resultArray[%u] = %f",
					old2new[i], current_array[old2new[i]], i, resultArray[i]);
			}
			else if constexpr (std::is_same_v<T, double>)
			{
				assert_msg((current_array[old2new[i]] == resultArray[i]), "error:current_array[%u] = %lf, resultArray[%u] = %lf",
					old2new[i], current_array[old2new[i]], i, resultArray[i]);
			}
			else
			{
