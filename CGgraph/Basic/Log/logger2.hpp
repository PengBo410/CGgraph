...
void textcolor(FILE *handle, int attr, int fg) {
    ...
    sprintf(command, "%c[%hhd;%hhdm", 0x1B, attr, fg + 30);
...
void setLogNFS(std::string logstr = "logInfo_default.txt", bool isConsole = false)
{
	//配置日志
	std::string basePath = "/home/pengjie/vs_log/";
	createFloder(basePath.c_str());
	global_logger().set_log_level(LOG_INFO);
	//std::string logstr = "logger/logInfo_1.txt";
	bool isopen = global_logger().set_log_file(basePath + logstr);//文件会自动创建
	if (!isopen) std::cout << "Log File Is Not Opened\n";
	global_logger().set_log_to_console(isConsole);
}