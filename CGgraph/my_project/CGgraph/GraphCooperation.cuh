...
//[OFFLOAD]
#define RESERVE_CHUNK_VEC 10000               
#define WORD_NUM_CHUNK (128)            
#define USED_DEVICE_RATE 0.1               
#define OFFLOAD_TASK_RATE 0.30                
#define OUR_DOUBLE_MAX 100000000.0 
#define DEVICE_PERFER 15                    
...
public:
	...
	double graphCooperationExecution(vertex_id_type root = 0)
	{
		...
		std::thread thread_device(&GraphCooperation::GPU_Execution_Model, this);
			...
			//[Three]: Host-compute
			{
				...
				activeNum_host = taskSteal->threeStage_taskSteal<GraphCooperation>(*this,
					[&](vertex_id_type vertex, countl_type nbr_start, countl_type nbr_end, count_type socketId, bool sameSocket)
					{
						...
						{
							return PR_SPACE::pr_numa_lastzero<GraphCooperation>(*this, vertex, nbr_start, nbr_end, socketId, sameSocket);
						}
                        else
                        {
	...
	void GPU_Execution_Model()
	{
		...
		while (!hybirdComplete)
		{
			...
			for (offset_type gpu_model_id = 0; gpu_model_id < gpu_model_num; gpu_model_id++)
			{
				...
				{
					CC_SPACE::cc_hybird_pcie_cooperation<GraphCooperation>(*this, nBlock);
				}
				else if (algorithm == Algorithm_type::PR)
				{
					PR_SPACE::pr_hybird_pcie_cooperation<GraphCooperation>(*this, nBlock);
				}
				else
				{
